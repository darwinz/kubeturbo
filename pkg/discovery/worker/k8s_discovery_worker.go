...
func NewK8sDiscoveryWorker(config *k8sDiscoveryWorkerConfig, wid string, globalMetricSink *metrics.EntityMetricSink,
	...
	for monitorType, configList := range config.monitoringSourceConfigs {
		...
		for _, config := range configList {
			monitoringWorker, err := monitoring.BuildMonitorWorker(config.GetMonitoringSource(), config, isFullDiscoveryWorker, globalMetricSink)
			if err != nil {
				// TODO return?
			...
			go func(w monitoring.MonitoringWorker) {
				...
				case <-t.C:
					glog.Errorf("%s monitoring worker from discovery worker %v exceeds the max time limit for "+
						"completing the task %+v: %v", w.GetMonitoringSource(), worker.id, resourceMonitorTask, timeoutSecond)
					timeout = true
					timeoutCh <- struct{}{}
...
func (worker *k8sDiscoveryWorker) buildEntityDTOs(currTask *task.Task) ([]*proto.EntityDTO,
	...
	nodeDTOs, notReadyNodes := worker.buildNodeDTOs([]*api.Node{currTask.Node()}, currTask.NodesPods(),
		currTask.HostnameSpreadPods(), currTask.HostnameSpreadWorkloads(), currTask.OtherSpreadPods(),
		currTask.PodstoControllers(), currTask.Cluster(), currTask.Node2NodeGroups(), currTask.AffinityMapper())
	...
	containerDTOs, sidecarContainerSpecs := worker.buildContainerDTOs(currTask.Cluster(), runningPods)
...
func (worker *k8sDiscoveryWorker) buildNodeDTOs(nodes []*api.Node, nodesPods map[string][]string,
	hostnameSpreadPods, hostnameSpreadWorkloads, otherSpreadPods sets.String, podsToControllers map[string]string,
	clusterSummary *repository.ClusterSummary, node2nodegroup map[string]sets.String, affinityMapper *podaffinity.AffinityMapper) ([]*proto.EntityDTO, []string) {
	...
	// Build entity DTOs for nodes
	return dtofactory.NewNodeEntityDTOBuilder(worker.sink, stitchingManager, clusterSummary, affinityMapper).
		WithClusterKeyInjected(worker.config.clusterKeyInjected).
		BuildEntityDTOs(nodes, nodesPods, hostnameSpreadPods, hostnameSpreadWorkloads, otherSpreadPods, podsToControllers, node2nodegroup)
}
...
func (worker *k8sDiscoveryWorker) buildPodDTOs(currTask *task.Task) ([]*proto.EntityDTO, []*api.Pod, []string, []string) {
	...
	runningPodDTOs, pendingPodDTOs, podsWithVolumes, mirrorPodUids := dtofactory.
		NewPodEntityDTOBuilder(worker.sink, currTask.Cluster(), worker.stitchingManager, worker.k8sClusterScraper).
		// Node providers
		...
		WithPodsWithLabelBasedAffinities(currTask.PodsWithLabelBasedAffinities()).
		...
		WithPodNonHostnameAntiTermTopologyKeys(currTask.PodNonHostnameAntiTermTopologyKeys()).
		WithNode2NodeGroups(currTask.Node2NodeGroups()).
		WithAffinityMapper(currTask.AffinityMapper()).
		BuildEntityDTOs()
...
func (worker *k8sDiscoveryWorker) buildContainerDTOs(cluster *repository.ClusterSummary, runningPods []*api.Pod) ([]*proto.EntityDTO, []string) {
	return dtofactory.
		NewContainerDTOBuilder(worker.sink, cluster).
...
func (worker *k8sDiscoveryWorker) buildAppDTOs(
	...
	applicationEntityDTOBuilder := dtofactory.
		NewApplicationEntityDTOBuilder(worker.sink, cluster, worker.k8sClusterScraper)
